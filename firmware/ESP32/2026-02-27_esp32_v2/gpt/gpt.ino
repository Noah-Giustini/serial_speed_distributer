...
void Handle_OBD(void *pvParameters){
  while(true){
    ...
    vTaskDelay(1000 / portTICK_PERIOD_MS); /////////////////////////// Change 2
  }
}

void Handle_Client(void *pvParameters){
  while(true){
    String temp; /////////////////////////// Change 3
    if (Client.available()){
      ...
      if (client.indexOf("010D") >= 0){
          {
          std::lock_guard<std::mutex> lck(serial_mtx);
          temp = speed_from_obd; /////////////////////////// Change 4
          //Serial.println("Speed Received: " +speed_from_obd);
          }
          Client.print(speed_from_obd); /////////////////////////// Change 5
      }
    }
/////////////////////////////////////////////////////////////////// Extra client here
    if (Serial.available()){
      String extraclient = Serial.readStringUntil('>');
      Serial.flush(); /////////////////////////// Change 6
      //Serial.println("Client message: " +client);
      if (extraclient.indexOf("010D") >= 0){
          {
          std::lock_guard<std::mutex> lck(serial_mtx);
          temp = speed_from_obd; /////////////////////////// Change 7
          //Serial.println("Speed Received: " +speed_from_obd);
          }
          Serial.print(speed_from_obd); /////////////////////////// Change 8
      }
    }
///////////////////////////////////////////////////////////////////
